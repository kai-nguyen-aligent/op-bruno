...
export default class Extract extends Command {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      console.log(
        chalk.bold('Step 1: Extracting secrets from Bruno environments...')
      );
      ...
      if (Object.keys(secrets).length === 0) {
        console.warn(
          chalk.yellow('⚠️  No secrets found in Bruno environment files')
        );
        console.log(
          chalk.gray(
            '   Make sure your secret variables are marked with ~secret flag'
          )
        );
        return;
      }
      ...
      console.log(
        chalk.green(`  • Updated collection.bru with pre-request script`)
      );

      if (flags['1password']) {
        console.log(
          chalk.green(
            `  • Created 1Password item "${flags.title}" in vault "${flags.vault}"`
          )
        );
      }
      ...
      if (!flags['1password']) {
        console.log(
          chalk.cyan(
            '  3. Consider creating a 1Password item with --1password flag'
          )
        );
      }
    } catch (error) {
