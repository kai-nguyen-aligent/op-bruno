...
import { SecretMap, OnePasswordOptions } from '../types/index.js'
...
export class OnePasswordManager {
  async checkCLI(): Promise<boolean> {
    try {
      ...
      console.warn(chalk.yellow('⚠️  1Password CLI not found. Please install it from: https://developer.1password.com/docs/cli'))
      return false
    }
  }
  ...
  async updateItem(
    ...
    console.log(chalk.blue(`📝 Updating 1Password item: ${itemName} in vault: ${vault}`))
    ...
    try {
      ...
      for (const [envName, envSecrets] of Object.entries(secrets)) {
        ...
        for (const [key, value] of Object.entries(envSecrets)) {
          updates.push(`op item edit "${itemName}" --vault="${vault}" '${sectionName}.${key}[password]=${value}'`)
        }
      }
  ...
  async verifyAccess(vault: string): Promise<boolean> {
    try {
      ...
      const { stdout } = await execAsync(`op item list --vault="${vault}" --limit=1`)
      return true
    } catch (error) {
      ...
      console.error(chalk.yellow(`  2. The vault "${vault}" exists and you have access to it`))
      return false
    }
  }
}