...
export class OnePasswordManager {
    ...
    async createItem(secrets: SecretMap, options: OnePasswordOptions): Promise<string | null> {
    ...
    async updateItem(itemName: string, vault: string, secrets: SecretMap): Promise<boolean> {
        // Check if CLI is available
        if (!(await this.checkCLI())) {
            return false;
        }
