...
export class CollectionGenerator {
  ...
  async updateCollection(
    ...
    if (await fs.pathExists(collectionPath)) {
      await this.modifyExistingCollection(
        collectionPath,
        secrets,
        itemName,
        vaultName
  ...
  private async modifyExistingCollection(
    ...
    if (collection.script?.req) {
      console.warn(
        chalk.yellow('⚠️  WARNING: Pre-request script already exists!')
      );
      console.warn(
        chalk.yellow(`   Please review the modifications at: ${collectionPath}`)
      );
      console.warn(
        chalk.yellow(
      ...
      collection.script.req = generatePreRequestScript(
        secrets,
        itemName,
        vaultName
      );
  ...
  private async createNewCollection(
    ...
    console.log(
      chalk.green('✓ Created collection.bru with pre-request script')
    );
  }
